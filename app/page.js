...
const CATEGORY_CONFIG = {
  brand_bag: {
    label: "ブランドバッグ", examples: "ルイヴィトン, シャネル, エルメス",
    fields: [
  ...
  brand_watch: {
    label: "腕時計", examples: "ロレックス, オメガ, カルティエ",
    fields: [
  ...
  jewelry: {
    label: "ジュエリー・宝石", examples: "ダイヤ, 金, プラチナ",
    fields: [
  ...
  brand_clothes: {
    label: "ブランド衣類", examples: "グッチ, プラダ, バーバリー",
    fields: [
  ...
  electronics: {
    ...
    fields: [
      ...
      { key: "battery", label: "バッテリー状態", placeholder: "例: 最大容量89%、充電回数200回" },
      { key: "operation", label: "動作状況", placeholder: "例: 全機能問題なし、カメラのAF不良あり" },
      { key: "lock", label: "ロック解除 / 初期化", placeholder: "例: iCloudサインアウト済み、初期化済み" },
      { key: "accessories", label: "付属品", placeholder: "箱, 充電器, ケーブル, 説明書" },
      { key: "condition_detail", label: "状態の詳細", placeholder: "画面の傷、バッテリー膨張、動作不良など", multiline: true },
    ],
    photoGuide: [
      "正面（画面が点灯した状態）",
      "背面",
  ...
  antique: {
    label: "骨董品・美術品", examples: "掛軸, 茶道具, 陶磁器",
    fields: [
      { key: "name", label: "品名 / 種類", placeholder: "例: 備前焼 花入、掛軸 山水図", required: true },
      { key: "artist", label: "作家名 / 窯元", placeholder: "例: 金重陶陽、不明" },
      { key: "era", label: "時代 / 年代", placeholder: "例: 江戸時代、明治期、昭和" },
      { key: "size", label: "サイズ", placeholder: "例: 高さ25cm、幅30cm" },
      { key: "material", label: "素材 / 技法", placeholder: "例: 陶器、絹本着色、木彫" },
      { key: "signature", label: "銘 / 落款 / 署名", placeholder: "あり・なし、内容" },
      { key: "provenance", label: "来歴 / 入手経緯", placeholder: "例: 祖父のコレクション、古美術商から購入" },
      { key: "cert", label: "鑑定書 / 箱書", placeholder: "共箱あり、鑑定書あり、なし" },
      { key: "accessories", label: "付属品", placeholder: "共箱, 仕覆, 鑑定書" },
      { key: "condition_detail", label: "状態の詳細", placeholder: "ヒビ、欠け、シミ、修復歴など", multiline: true },
    ],
    photoGuide: [
      "全体の写真（正面）",
      "裏面・底面の写真",
  ...
  musical: {
    ...
    fields: [
      { key: "brand", label: "メーカー", placeholder: "例: Gibson, Fender, YAMAHA", required: true },
      { key: "name", label: "モデル名 / 型番", placeholder: "例: Les Paul Standard '60s", required: true },
      { key: "year", label: "製造年", placeholder: "例: 2019年製、ビンテージ1965年" },
      { key: "serial", label: "シリアルナンバー", placeholder: "ヘッド裏やラベルに記載" },
      { key: "specs", label: "仕様", placeholder: "例: マホガニーボディ, ローズ指板, PAFピックアップ" },
      { key: "operation", label: "動作状況", placeholder: "例: ネック反りなし、フレット残90%、電装系問題なし" },
      { key: "accessories", label: "付属品", placeholder: "ハードケース, ストラップ, ケーブル" },
      { key: "condition_detail", label: "状態の詳細", placeholder: "打痕、塗装の剥がれ、フレットの摩耗など", multiline: true },
    ],
    photoGuide: [
      "全体の正面写真",
      "背面の写真",
  ...
  liquor: {
    ...
    fields: [
      { key: "brand", label: "銘柄", placeholder: "例: 山崎25年、ロマネコンティ", required: true },
      { key: "name", label: "商品名 / 種類", placeholder: "例: シングルモルトウイスキー", required: true },
      { key: "vintage", label: "ヴィンテージ / 蒸溜年", placeholder: "例: 2005年、NV" },
      { key: "volume", label: "容量", placeholder: "例: 700ml、750ml" },
      { key: "abv", label: "アルコール度数", placeholder: "例: 43%" },
      { key: "storage", label: "保管状況", placeholder: "例: 冷暗所保管、セラー保管、常温" },
      { key: "level", label: "液面レベル", placeholder: "例: ラベル上部まで、減りあり" },
      { key: "accessories", label: "付属品", placeholder: "箱, 冊子, 替え栓" },
      { key: "condition_detail", label: "状態の詳細", placeholder: "ラベルの汚れ、キャップの状態、澱の有無など", multiline: true },
    ],
    photoGuide: [
      "ボトル全体（ラベルが見える正面）",
      "ラベルのアップ",
  ...
  gold_metal: {
    label: "金・貴金属", examples: "金地金, 金貨, プラチナ",
    fields: [
      { key: "name", label: "品名 / 種類", placeholder: "例: 金地金(インゴット)、メイプルリーフ金貨", required: true },
      { key: "metal_type", label: "金属の種類 / 純度", placeholder: "例: K24(純金)、K18、Pt1000" },
      { key: "weight", label: "重量", placeholder: "例: 100g、1oz（31.1g）", required: true },
      { key: "maker", label: "製造元 / ブランド", placeholder: "例: 田中貴金属、PAMP、造幣局" },
      { key: "serial", label: "シリアルナンバー（あれば）", placeholder: "インゴットに刻印" },
      { key: "cert", label: "品位証明 / 鑑定書", placeholder: "あり / なし" },
      { key: "accessories", label: "付属品", placeholder: "ケース, 保証書, アッセイカード" },
      { key: "condition_detail", label: "状態の詳細", placeholder: "傷、変色、刻印の鮮明さなど", multiline: true },
    ],
    photoGuide: [
      "表面の全体写真",
      "裏面の写真",
  ...
  other: {
    ...
    fields: [
      { key: "brand", label: "ブランド / メーカー", placeholder: "例: バカラ、リヤドロ、バンダイ" },
      { key: "name", label: "商品名", placeholder: "例: ロックグラス、ワンピース フィギュア", required: true },
      { key: "material", label: "素材", placeholder: "例: クリスタル、PVC" },
      { key: "size", label: "サイズ", placeholder: "例: 高さ20cm" },
      { key: "year", label: "年代 / 発売時期", placeholder: "例: 2022年発売" },
      { key: "accessories", label: "付属品", placeholder: "箱, 説明書, 保証書" },
      { key: "condition_detail", label: "状態の詳細", placeholder: "傷、欠け、動作不良、開封/未開封など", multiline: true },
    ],
    photoGuide: [
      "全体の写真",
      "ブランドロゴ / 刻印部分",
...
async function callAppraisal({ category, fields, condition, imageBase64 }) {
  ...
  p += '\n\n以下のJSON形式のみで回答してください。他の文字列は一切含めないでください。売却先候補は必ずウェブ検索で現在実在する業者名を調べてください:\n{"identified_item":"特定した商品","min_price":数値,"max_price":数値,"recommended_price":数値,"market_price":数値,"reasoning":"査定根拠","min_price_reason":"下限理由","max_price_reason":"上限理由","price_factors":["要因1","要因2"],"market_trend":"上昇/安定/下降","trend_reason":"理由","confidence":"高/中/低","sales_channels":[{"name":"売却先名","type":"種別","expected_sell_price":数値,"gross_profit":数値,"gross_profit_rate":"率","priority":数値,"reason":"理由"}],"purchase_upper_limit":数値,"purchase_limit_reason":"根拠","inventory_strategy":"在庫向き/即売り向き","inventory_reason":"理由","recommended_channel":"推奨先","admin_memo":"管理者メモ","customer_explanation":"お客様説明文3-4文"}';
  ...
  if (!textResult) throw new Error("査定結果を取得できませんでした");
  const cleaned = textResult.replace(/```json|```/g, "").trim();
  const jsonMatch = cleaned.match(/\{[\s\S]*\}/);
  if (!jsonMatch) throw new Error("査定結果の解析に失敗しました");
...
function Box({ title, sub, children, type }) {
  const bg = {admin:cl.adminBg,staff:cl.staffBg}[type]||cl.surface;
  const bd = {admin:cl.adminBorder,staff:cl.staffBorder}[type]||cl.border;
  const co = {admin:cl.adminAccent,staff:cl.staffAccent}[type]||cl.accent;
  return (
...
function ImageUploader({ images, setImages, photoGuide }) {
  ...
  return (
    <div style={{ marginBottom:14 }}>
      <div onClick={()=>fileRef.current?.click()} style={{ border:"2px dashed "+cl.border, borderRadius:8, padding:"20px 16px", textAlign:"center", cursor:"pointer", background:cl.surfAlt }}>
        <div style={{ width:34, height:34, margin:"0 auto 6px", borderRadius:"50%", border:"2px solid "+cl.textD, display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, color:cl.textD }}>+</div>
      ...
      {photoGuide && photoGuide.length > 0 && (
        <div style={{ marginTop:12, padding:"12px 14px", background:"#F8F6F1", borderRadius:6, border:"1px solid "+cl.border }}>
          <p style={{ margin:"0 0 8px", fontSize:11, fontWeight:700, color:cl.accent, fontFamily:font }}>撮影ガイド（この商品に必要な写真）</p>
          {photoGuide.map((g,i)=>(
            <div key={i} style={{ display:"flex", alignItems:"flex-start", gap:6, marginBottom:4 }}>
              <span style={{ fontSize:11, color:cl.accentL, fontFamily:font, flexShrink:0 }}>{i+1}.</span>
              <span style={{ fontSize:11, color:cl.textM, fontFamily:font, lineHeight:1.5 }}>{g}</span>
            </div>
          ))}
        </div>
      )}
...
function StaffExplanation({ result }) {
  ...
  if (!text) return null;
  return (
    <Box title="お客様への説明文" sub="このままお伝えいただける内容です" type="staff">
      ...
      <button onClick={()=>{navigator.clipboard.writeText(text).then(()=>{setCopied(true);setTimeout(()=>setCopied(false),2000)})}} style={{ background:"none", border:"1px solid "+cl.staffBorder, borderRadius:6, padding:"8px 16px", fontSize:12, fontWeight:600, color:cl.staffAccent, cursor:"pointer", fontFamily:font, width:"100%" }}>
        ...
        {channels.sort((a,b)=>a.priority-b.priority).map((ch,i)=>(
          <div key={i} style={{ padding:14, background:cl.white, borderRadius:8, border:"1px solid "+cl.adminBorder, marginBottom:i<channels.length-1?10:0 }}>
            <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:8 }}>
              <span style={{ width:22, height:22, borderRadius:"50%", background:cl.adminAccent, color:cl.white, display:"inline-flex", alignItems:"center", justifyContent:"center", fontSize:11, fontWeight:700, fontFamily:font }}>{ch.priority}</span>
              <div><p style={{ margin:0, fontSize:14, fontWeight:700, color:cl.text, fontFamily:font }}>{ch.name}</p><p style={{ margin:"2px 0 0", fontSize:11, color:cl.textD, fontFamily:font }}>{ch.type}</p></div>
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:8 }}>
              <div style={{ padding:"6px 10px", background:cl.adminBg, borderRadius:4 }}><p style={{ margin:0, fontSize:10, color:cl.textD, fontFamily:font }}>想定売価</p><p style={{ margin:"2px 0 0", fontSize:14, fontWeight:700, color:cl.text, fontFamily:font }}>{fmt(ch.expected_sell_price)}</p></div>
              <div style={{ padding:"6px 10px", background:ch.gross_profit>0?"#F0F7F2":"#FDF5F4", borderRadius:4 }}><p style={{ margin:0, fontSize:10, color:cl.textD, fontFamily:font }}>粗利見込み</p><p style={{ margin:"2px 0 0", fontSize:14, fontWeight:700, color:ch.gross_profit>0?cl.success:cl.danger, fontFamily:font }}>{fmt(ch.gross_profit)}</p><p style={{ margin:"1px 0 0", fontSize:10, color:cl.textD, fontFamily:font }}>{ch.gross_profit_rate}</p></div>
            </div>
    ...
    <>
      ...
      <div style={{ background:cl.surface, borderRadius:10, border:"1px solid "+cl.accentL, padding:20, marginBottom:14 }}>
        ...
        <div style={{ marginBottom:18 }}>
          <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}><span style={{ fontSize:11, color:cl.textD, fontFamily:font }}>下限</span><span style={{ fontSize:11, color:cl.textD, fontFamily:font }}>上限</span></div>
          ...
          <div style={{ display:"flex", justifyContent:"space-between", marginTop:6 }}><span style={{ fontSize:14, fontWeight:700, color:cl.text, fontFamily:font }}>{fmt(result.min_price)}</span><span style={{ fontSize:14, fontWeight:700, color:cl.text, fontFamily:font }}>{fmt(result.max_price)}</span></div>
        </div>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:16 }}>
          <div style={{ padding:"10px 12px", background:"#FDF5F4", borderRadius:6, borderLeft:"3px solid "+cl.danger }}><p style={{ margin:0, fontSize:10, fontWeight:700, color:cl.danger, fontFamily:font }}>下限の理由</p><p style={{ margin:"4px 0 0", fontSize:11, color:cl.text, lineHeight:1.6, fontFamily:font }}>{result.min_price_reason||"—"}</p></div>
          <div style={{ padding:"10px 12px", background:"#F2F8F4", borderRadius:6, borderLeft:"3px solid "+cl.success }}><p style={{ margin:0, fontSize:10, fontWeight:700, color:cl.success, fontFamily:font }}>上限の理由</p><p style={{ margin:"4px 0 0", fontSize:11, color:cl.text, lineHeight:1.6, fontFamily:font }}>{result.max_price_reason||"—"}</p></div>
        </div>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8 }}>
          {[["販売相場",fmt(result.market_price),cl.text],["トレンド",trend(result.market_trend),cl.text],["信頼度",result.confidence,result.confidence==="高"?cl.success:result.confidence==="低"?cl.danger:cl.accent]].map(([l,v,co])=>(
            <div key={l} style={{ padding:"8px 10px", background:cl.surfAlt, borderRadius:6 }}><p style={{ fontSize:10, color:cl.textD, margin:0, fontFamily:font }}>{l}</p><p style={{ fontSize:13, fontWeight:600, color:co, margin:"2px 0 0", fontFamily:font }}>{v}</p></div>
          ))}
        </div>
      </div>
      <Box title="査定根拠">
...
function AppraisalTab({ role, onSaveToLegal }) {
  ...
  const hasRequired = config ? config.fields.filter(f=>f.required).every(f=>fields[f.key]?.trim()) : false;
  const ok = catId && (hasImg || hasRequired);
  ...
  const run = async () => {
    ...
    try {
      const condLabel = CONDITIONS.find(x=>x.id===cond)?.label||cond;
      const data = await callAppraisal({ category:config?.label||"", fields, condition:condLabel, imageBase64:images[0]?.base64||null });
      setResult(data);
    } catch(e) { setError(e.message||"査定に失敗しました。もう一度お試しください。"); }
    setLoading(false);
  };
  ...
  return (
    <div style={{ padding:16 }}>
      {/* 1. Category */}
      <Box title="カテゴリを選択" sub="商品の種類を選んでください">
        <div style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:6 }}>
          {Object.entries(CATEGORY_CONFIG).map(([id,cfg])=>(
            <button key={id} onClick={()=>{setCatId(id);setFields({});setResult(null)}} style={{ padding:"9px 10px", textAlign:"left", cursor:"pointer", background:catId===id?cl.accentBg:cl.surfAlt, border:"1px solid "+(catId===id?cl.accentL:cl.border), borderRadius:6 }}>
      ...
      {/* 2. Photo with guide (shown after category selected) */}
      {catId && (
        <Box title="商品写真" sub="写真があると査定精度が上がります">
      ...
      {catId && config && (
        <Box title={config.label + "の詳細"} sub="わかる範囲で入力してください">
          {config.fields.map(f => (
            <Input key={f.key} label={f.label} value={fields[f.key]||""} onChange={v=>updateField(f.key,v)} placeholder={f.placeholder} required={f.required} multiline={f.multiline} />
          ))}
          <Sel label="状態ランク" value={cond} onChange={setCond} options={CONDITIONS.map(x=>({value:x.id,label:x.label+" — "+x.desc}))} />
        </Box>
      )}

      {catId && <Btn onClick={run} disabled={!ok} loading={loading}>AI査定を実行</Btn>}
      {catId && !ok && <p style={{ fontSize:11, color:cl.textD, textAlign:"center", marginTop:8, fontFamily:font }}>写真を追加するか、必須項目（*）を入力してください</p>}
...
function HistoryTab({ records, role }) {
  ...
  if(sel){const r=sel;return(
    <div style={{padding:16}}>
      ...
      <Box title="取引詳細">{[["取引日",r.date],["取引場所",r.transactionLocation],["担当者",r.staffName],["伝票番号",r.receiptNumber]].map(([l,v])=><Row key={l} label={l} value={v}/>)}</Box>
      <Box title="商品情報">{[["品目",r.itemCategory],["ブランド",r.itemBrand],["商品名",r.itemName],["状態",r.itemCondition],["付属品",r.itemAccessories],["買取金額",fmt(r.purchasePrice)]].map(([l,v])=><Row key={l} label={l} value={v}/>)}</Box>
      {role==="admin"&&<Box title="相手方情報" type="admin">{[["氏名",r.sellerName],["フリガナ",r.sellerNameKana],["生年月日",r.sellerDob],["住所",r.sellerAddress],["電話番号",r.sellerPhone]].map(([l,v])=><Row key={l} label={l} value={v} borderColor={cl.adminBorder}/>)}</Box>}
      {role==="admin"&&<Box title="本人確認" type="admin">{[["証明書種類",r.sellerIdType],["証明書番号",r.sellerIdNumber],["確認者",r.idVerifiedBy],["確認方法",r.idVerifyMethod]].map(([l,v])=><Row key={l} label={l} value={v} borderColor={cl.adminBorder}/>)}</Box>}
    </div>
  );}
  return(
    <div style={{padding:16}}>
    ...
    <div style={{padding:16}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        {[{l:"総取引件数",v:records.length+"件"},{l:"総買取金額",v:fmt(total)},{l:"本日の件数",v:todayR.length+"件"},{l:"本日の買取額",v:fmt(todayT)}].map(({l,v})=>(
          <div key={l} style={{background:cl.adminBg,borderRadius:8,padding:"12px 14px",border:"1px solid "+cl.adminBorder}}><p style={{margin:0,fontSize:10,color:cl.adminAccent,fontWeight:600,fontFamily:font}}>{l}</p><p style={{margin:"4px 0 0",fontSize:18,fontWeight:800,color:cl.text,fontFamily:font}}>{v}</p></div>
        ))}
      </div>
    </div>
  );
...
function LoginScreen({ onLogin }) {
  ...
  return(
    <div style={{maxWidth:400,margin:"0 auto",minHeight:"100vh",background:cl.bg,fontFamily:font,display:"flex",flexDirection:"column",justifyContent:"center",padding:24}}>
      <div style={{textAlign:"center",marginBottom:36}}><h1 style={{fontSize:20,fontWeight:800,color:cl.accent,margin:"0 0 4px",letterSpacing:2}}>出張買取</h1><p style={{fontSize:12,color:cl.textD,margin:0}}>AI査定 ・ 古物台帳記録システム</p></div>
      <Box title="ログイン">
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:18}}>
          {[["admin","管理者","全データ閲覧・管理",cl.adminBg,cl.adminAccent],["staff","スタッフ","査定・台帳入力",cl.accentBg,cl.accent]].map(([id,label,desc,bg,co])=>(
            <button key={id} onClick={()=>setRole(id)} style={{padding:"16px 12px",background:role===id?bg:cl.surfAlt,border:"2px solid "+(role===id?co:cl.border),borderRadius:8,cursor:"pointer",textAlign:"center"}}><p style={{margin:0,fontSize:14,fontWeight:700,color:role===id?co:cl.text,fontFamily:font}}>{label}</p><p style={{margin:"4px 0 0",fontSize:11,color:cl.textD,fontFamily:font}}>{desc}</p></button>
          ))}
        </div>
        {role&&<><Input label="PIN" value={pin} onChange={setPin} placeholder="PINを入力" type="password" required /><Btn onClick={go} disabled={pin.length<4} variant={role==="admin"?"admin":"primary"}>ログイン</Btn>{err&&<p style={{fontSize:12,color:cl.danger,textAlign:"center",marginTop:8,fontFamily:font}}>{err}</p>}</>}
      </Box>
    </div>
  );
}
