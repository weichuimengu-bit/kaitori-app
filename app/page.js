...
function BulkResultCard({ result, index }) {
  ...
  return (
    <div style={{background:cl.surface,borderRadius:8,border:"1px solid "+cl.border,marginBottom:10,overflow:"hidden"}}>
      <div onClick={()=>setOpen(o=>!o)} style={{padding:"12px 14px",display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer",background:cl.surfAlt}}>
    ...
    <div style={{padding:16}}>
      ...
      {mode==="single" && <>
        ...
        {result && <div style={{marginTop:20}}>
          ...
          <div style={{marginTop:12}}><Btn onClick={sendSingleToLedger} variant="secondary">📋 この査定内容を台帳に引き継ぐ</Btn></div>
        </div>}
      </>}
...
function LegalTab({ staffName, pendingLedger, onPendingConsumed }) {
  ...
  useEffect(()=>{
    ...
    if (pendingLedger.items?.length>0) {
      setItems(pendingLedger.items.map(it=>({
        category:it.category||"",name:it.name||"",quantity:it.quantity||"1",
        feature:it.feature||"",purchasePrice:it.purchasePrice||"",remarks:it.remarks||"",
        aiEstimate:it.aiEstimate||"",
        aiMinPrice:it.aiMinPrice||null,aiMaxPrice:it.aiMaxPrice||null,aiRecommendedPrice:it.aiRecommendedPrice||null,
      })));
    }
    setShowBanner(true);
    onPendingConsumed();
  ...
  const save = async () => {
    ...
    try {
      const data = await gasPost({
        ...
        items:items.map(it=>({
          category:it.category,name:it.name,quantity:it.quantity||"1",
          feature:it.feature,purchasePrice:it.purchasePrice,
          remarks:it.remarks,aiEstimate:it.aiEstimate,
        })),
      });
  ...
  return (
    <div style={{padding:16}}>
      {showBanner && (
        <div style={{padding:"12px 14px",background:"#F0F7F2",border:"1px solid "+cl.staffBorder,borderRadius:8,marginBottom:14,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <p style={{margin:0,fontSize:12,color:cl.staffAccent,fontWeight:600,fontFamily:font}}>📋 査定画面の内容を反映しました。手修正してから保存してください。</p>
          <button onClick={()=>setShowBanner(false)} style={{background:"none",border:"none",fontSize:16,cursor:"pointer",color:cl.textD}}>×</button>
        </div>
      )}
