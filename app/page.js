...
function getLocation() {
  return new Promise((resolve) => {
    if (!navigator.geolocation) {
      resolve({ success: false, errorReason: "この端末は位置情報に対応していません" });
      return;
    }
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        ...
        resolve({ success: false, errorReason: reasons[err.code] || "不明なエラー" });
      },
...
async function reverseGeocode(lat, lng) {
  try {
    const res = await fetch(
      ...
      { headers: { "User-Agent": "KaitoriApp/1.0" } }
    );
    if (!res.ok) return "";
    const data = await res.json();
...
async function sendLocationLog({ staffId, staffName, caseId, actionType, remarks }) {
  try {
    ...
    await gasPost({
      ...
      caseId: caseId || "",
      actionType,
      locationSuccess: loc.success,
      latitude: loc.success ? loc.latitude : null,
      longitude: loc.success ? loc.longitude : null,
      accuracy: loc.success ? loc.accuracy : null,
      address,
      errorReason: loc.success ? "" : loc.errorReason,
      remarks: remarks || "",
    });
    return loc.success;
  } catch {
...
const CATEGORY_CONFIG = {
  brand_bag: {
    ...
    authGuide: { summary: "「素材の質感」「縫製の精度」「金具の刻印」の3点を重点確認。", checks: ["素材の質感と匂い — 本革特有のしっとり感","縫製 — 糸の間隔が均一か","金具の刻印 — 彫りが浅い・にじむものは偽物の可能性","シリアル — 書体やフォーマットが正規品と一致するか","ロゴ印字 — にじみ・ズレがないか"] },
  },
  brand_watch: {
...
async function callAppraisal({ category, fields, condition, image }) {
  ...
  if (image && image.base64 && image.mimeType && supportedImageTypes.includes(image.mimeType)) p += "添付画像も参考にしてください。\n\n";
  p += "カテゴリ: " + (category || "不明") + "\n状態: " + (condition || "不明") + "\n\n商品詳細:\n" + (ft || "画像から判断");
...
function Checkbox({ label, checked, onChange, disabled }) {
  return (<label style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", cursor: disabled ? "not-allowed" : "pointer", fontFamily: font, fontSize: 13, color: cl.text, opacity: disabled ? 0.5 : 1 }}><input type="checkbox" checked={checked} onChange={e => onChange(e.target.checked)} disabled={disabled} style={{ width: 18, height: 18, accentColor: cl.accent }} />{label}</label>);
}
...
function LocationStatus({ status }) {
  ...
  const isOk = status === "取得済み";
  return (
    <div style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "4px 10px", borderRadius: 20, fontSize: 11, fontFamily: font, background: isOk ? "#F0F7F2" : "#FDF2F0", color: isOk ? cl.success : cl.danger, border: "1px solid " + (isOk ? cl.staffBorder : "#E8C4BF") }}>
      <span style={{ width: 6, height: 6, borderRadius: "50%", background: isOk ? cl.success : cl.danger }} />
      {isOk ? "位置情報取得済み" : "位置情報取得失敗"}
    </div>
  );
}
...
function PriceResult({ result }) {
  ...
  return (<>
    ...
    {result.auth_warning && <div style={{ marginBottom: 14, padding: 12, background: cl.authBg, borderRadius: 8, border: "1px solid " + cl.authBorder }}><p style={{ margin: 0, fontSize: 12, fontWeight: 700, color: cl.authAccent, fontFamily: font }}>⚠ 真贋注意</p><p style={{ margin: "4px 0 0", fontSize: 12, color: cl.text, lineHeight: 1.7, fontFamily: font }}>{result.auth_warning_reason || "真贋確認が必要な可能性があります。"}</p></div>}
...
function AppraisalTab({ staffName }) {
  ...
  const [catId, setCatId] = useState(""); const [fields, setFields] = useState({}); const [cond, setCond] = useState("");
  const [images, setImages] = useState([]); const [loading, setLoading] = useState(false); const [result, setResult] = useState(null); const [error, setError] = useState("");
  const config = CATEGORY_CONFIG[catId]; const hasImg = images.length > 0;
  const hasReq = config ? config.fields.filter(f => f.required).every(f => fields[f.key]?.trim()) : false;
  const ok = catId && (hasImg || hasReq);
  const run = async () => {
    setLoading(true); setError(""); setResult(null);
    try {
      const data = await callAppraisal({ category: config?.label || "", fields, condition: CONDITIONS.find(x => x.id === cond)?.label || cond, image: images[0] ? { base64: images[0].base64, mimeType: images[0].mimeType } : null });
      setResult(data);
      try { await gasPost({ action: "saveAppraisalLog", customerName: customerName.trim(), itemName: data.identified_item || fields.name || "", priceEstimate: fmt(data.recommended_price), staffName: staffName || "", remarks: config?.label || "" }); } catch {}
    } catch (e) { setError(e.message || "査定失敗"); }
    setLoading(false);
  };
  return (<div style={{ padding: 16 }}>
    <Box title="査定情報"><Input label="顧客名" value={customerName} onChange={setCustomerName} placeholder="例: 山田 太郎" /></Box>
...
function LegalTab({ user }) {
  ...
  const [locStatus, setLocStatus] = useState(null); // "取得済み" | "取得失敗" | null
  ...
  const resetForm = () => {
    ...
    setItems([{ ...emptyItem }]);
    setCaseId(""); setVisitStarted(false); setLocStatus(null);
  };

  // ── 訪問開始（位置情報取得） ──
  const handleVisitStart = async () => {
    setLocStatus(null);
    const ok = await sendLocationLog({ staffId, staffName, caseId: "", actionType: "訪問開始", remarks: visit.sellerName ? "顧客:" + visit.sellerName : "" });
    setLocStatus(ok ? "取得済み" : "取得失敗");
    setVisitStarted(true);
  };

  // ── 訪問終了（位置情報取得） ──
  const handleVisitEnd = async () => {
    await sendLocationLog({ staffId, staffName, caseId, actionType: "訪問終了", remarks: visit.sellerName ? "顧客:" + visit.sellerName : "" });
    resetForm();
    setSaved("訪問終了を記録しました");
  };
  ...
  const save = async (asContract) => {
    ...
    try {
      const data = await gasPost({
        ...
        sellerName: visit.sellerName, sellerPhone: visit.sellerPhone, sellerAddress: visit.sellerAddress,
        sellerIdType: visit.sellerIdType, sellerIdNumber: visit.sellerIdNumber,
        idVerifyMethod: "目視確認", idVerifiedBy: visit.idVerifiedBy,
        idVerified, staffName: visit.staffName,
        coolingOffExplained: coolingOff,
        additionalItemCheck: additionalCheck,
        additionalItemName: additionalCheck === "追加あり" ? additionalName : "",
        revisitType, revisitDate: revisitType !== "なし" ? revisitDate : "",
        contractComplete: asContract,
        items: items.map(it => ({
          itemName: it.itemName, purchasePrice: it.purchasePrice,
          remarks: "身分証:" + visit.sellerIdType + " " + visit.sellerIdNumber + " / 確認者:" + visit.idVerifiedBy + (it.remarks ? " / " + it.remarks : ""),
          status: it.status, authWarning: it.authWarning, photoUrls: it.photoUrls || [],
        }))
      });
      ...
      // 契約完了時に位置情報を送信
      if (asContract) {
        await sendLocationLog({ staffId, staffName, caseId: newCaseId, actionType: "契約完了", remarks: visit.sellerName });
      }

      setSaved((asContract ? "契約完了" : "保存完了") + "（案件ID: " + newCaseId + "）");
      if (asContract) {
        // 契約完了時はフォームリセットしない（訪問終了ボタンを使うため）
      }
  ...
  return (<div style={{ padding: 16 }}>
    ...
    {!visitStarted ? (
      <div style={{ marginBottom: 14 }}>
        <Btn onClick={handleVisitStart} variant="primary">📍 訪問開始（位置情報を記録）</Btn>
        <p style={{ fontSize: 11, color: cl.textD, textAlign: "center", marginTop: 6, fontFamily: font }}>訪問先に到着したら押してください</p>
      </div>
    ) : (
      <div style={{ marginBottom: 14, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <LocationStatus status={locStatus} />
        {caseId && <Btn onClick={handleVisitEnd} variant="danger">訪問終了</Btn>}
      </div>
    )}
      ...
      <Box key={idx} title={"商品 " + (idx + 1)}>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 6 }}>
            {STATUSES.map(s => (<button key={s.id} onClick={() => ui(idx, "status", s.id)} style={{ padding: "8px 10px", border: "1px solid " + (it.status === s.id ? s.color : cl.border), borderRadius: 6, background: it.status === s.id ? s.color + "18" : cl.surfAlt, cursor: "pointer", textAlign: "center" }}><span style={{ fontSize: 12, fontWeight: 600, color: it.status === s.id ? s.color : cl.text, fontFamily: font }}>{s.label}</span></button>))}
          </div>
        </div>
        {it.status === "管理者確認" && (
    ...
    <Box title="追加商材確認">
      ...
      {additionalCheck === "追加あり" && <Input label="追加商品名" value={additionalName} onChange={setAdditionalName} placeholder="追加商品の名称を入力" required />}
    </Box>

    <Box title="再訪・追客予定">
      <Sel label="再訪区分" value={revisitType} onChange={setRevisitType} options={REVISIT_OPTIONS} />
      {revisitType !== "なし" && <Input label={revisitType === "再訪予定あり" ? "再訪予定日" : "追客予定日"} value={revisitDate} onChange={setRevisitDate} type="date" />}
    </Box>

    <div style={{ marginBottom: 8 }}><Btn onClick={() => save(false)} disabled={saving || !basicOk} loading={saving}>一時保存（下書き）</Btn></div>
    <div style={{ marginBottom: 14 }}><Btn onClick={() => save(true)} disabled={saving || !contractOk} loading={saving} variant="success">契約完了として保存</Btn></div>
...
function LoginScreen({ onLogin }) {
  const [staffId, setStaffId] = useState(""); const [password, setPassword] = useState("");
  ...
  const go = async () => {
    ...
    try {
      ...
      const userData = { staffId: data.staffId, staffName: data.staffName };
      // ログイン成功時に位置情報を送信
      sendLocationLog({ staffId: data.staffId, staffName: data.staffName, caseId: "", actionType: "ログイン" });
      onLogin(userData);
  ...
  return (
    <div style={{ maxWidth: 400, margin: "0 auto", minHeight: "100vh", background: cl.bg, fontFamily: font, display: "flex", flexDirection: "column", justifyContent: "center", padding: 24 }}>
      ...
      <p style={{ fontSize: 10, color: cl.textD, textAlign: "center", marginTop: 16, fontFamily: font }}>ログイン時に位置情報を取得します</p>
    </div>
  );
}
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: cl.bg, fontFamily: font }}>
      ...
      <div style={{ display: "flex", background: cl.surface, borderBottom: "1px solid " + cl.border, position: "sticky", top: 0, zIndex: 100 }}>
        {tabs.map(t => (<button key={t.id} onClick={() => setTab(t.id)} style={{ flex: 1, padding: "11px 0", background: "none", border: "none", borderBottom: tab === t.id ? "2px solid " + cl.accent : "2px solid transparent", color: tab === t.id ? cl.accent : cl.textD, fontSize: 12, fontWeight: tab === t.id ? 700 : 400, cursor: "pointer", fontFamily: font }}>{t.label}</button>))}
      </div>
